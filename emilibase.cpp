...
emili::Solution* emili::BestImprovementSearch::search(emili::Solution* initial)
{
        ...
        if(incumbent->termcriterion == nullptr){
            incumbent->termcriterion = termcriterion;
        }
        ...
        do
        {
            *bestSoFar = *incumbent;
            ...
            delete ithSolution;
            if(incumbent->termcriterion->terminate(bestSoFar, incumbent))
                break;
...
/*emili::Solution* emili::BestImprovementSearch::search(emili::Solution* initial)
{
        ...
        {                       
            *bestSoFar = *incumbent;            
            Neighborhood::NeighborhoodIterator iter = neighbh->begin(bestSoFar);
            ithSolution = *iter;
	    //std::cout<<"solution value_ : "<< incumbent->getSolutionValue()<<std::endl;
            for(;iter!=end;++iter)
            {
                if(incumbent->operator >( *ithSolution)){                    
                    *incumbent = *ithSolution;
...
emili::Solution* emili::FirstImprovementSearch::search(emili::Solution* initial)
{
	    ...
	    //std::cout<<"solution value_ : "<< incumbent->getSolutionValue()<<std::endl;
            for(;iter!=end;++iter)
            {               
               // std::cout << "it " << incumbent->getSolutionRepresentation() << incumbent->getSolutionValue() << 
              //               " found " << ithSolution->getSolutionValue() <<  std::endl;
                if(incumbent->operator >(*ithSolution)){
                    *incumbent  = *ithSolution;
                    printSolstats(incumbent);
                    break;
...
emili::Solution* emili::GeneralTabuSearch::search(emili::Solution *initial)
{
    termcriterion->reset();
    neighbh->reset();
    emili::Solution* incumbent = initial->clone();
    emili::Solution* ithSolution;
        do{
            ...
            ithSolution = ls.search(bestSoFar);
            *incumbent = *ithSolution;
            if(incumbent->operator >(*ithSolution)&& (tabuMemory.tabu_check(ithSolution) || *ithSolution < *bestSoFar)){                    
                    tabuMemory.registerMove(incumbent,ithSolution);
                    *incumbent = *ithSolution;
...
emili::Solution* emili::GeneralTabuSearch::search(){
    termcriterion->reset();
...
emili::Solution* emili::RandomMovePerturbation::perturb(Solution *solution)
{
    ...
    for (int var = 1; var < numberOfSteps; ++var) {
        ...
        std::cout << typeid(explorer).name() << std::endl;
        delete temp;
...
emili::Solution* emili::SecuencialPerturbationSet::perturb(Solution *solution)
{
	Solution *aux;
	for(int i = 0; i < perturbations.size(); i++){
		//std::cout <<"perturbacion "<< i << std::endl;
		aux = perturbations[i]->perturb(solution);
	}
	return aux;
}
...
emili::Solution* emili::IteratedLocalSearch::search(emili::Solution* initial){
    termcriterion->reset();
    acc.reset();
    if(initial->termcriterion == nullptr){
        initial->termcriterion = &ls.getTermination();
    }
    ...
    *bestSoFar = *s;
    ...
    do{
        ...
        if(s->termcriterion == nullptr){
            s_p->termcriterion = s->termcriterion;
        }
        ...
        if(s_s->termcriterion == nullptr){
            s_s->termcriterion = s->termcriterion;
        }
        ...
        if(s->termcriterion != nullptr && s->termcriterion->terminate(s_p,s)){
            break;
        }
...
bool emili::MaxStepsTermination::terminate(Solution *currentSolution, Solution *newSolution)
{
    ...
    if(current_step >= max_steps_){
    	//std::cout <<"esto es 1"<< std::endl;

        if(*currentSolution > *newSolution)
        {
	    //std::cout <<"esto es 2"<< std::endl;
            *currentSolution = *newSolution;
	    //std::cout <<"esto es 3"<< std::endl;
        }
	//std::cout <<"esto es 4"<< std::endl;

        return true;
    }
    else
    {
	//std::cout <<"esto es 5"<< std::endl;
        current_step++;
	//std::cout <<"esto es 6"<< std::endl;

        return false;
    }
}

bool emili::MaxEvaluations::terminate(Solution *currentSolution, Solution *newSolution)
{
    if(current_eva >= max_eva_){
...
void emili::MaxEvaluations::reset()
{
    current_eva = 0;
}
...
emili::Solution* emili::MetropolisAcceptance::accept(Solution *intensification_solution, Solution *diversification_solution)
{
	...
	/*std::cout << "esto es prob" <<std::endl;
...
emili::Solution* emili::NeighborhoodShake::shake(Solution *s, int n)
{
    int k = n%n_num;
    int size = n/n_num + 1 ;
    std::cout << k <<" "<<shakes.size()<<std::endl;
    Neighborhood* p = shakes[k];
    return p->random(s,size);
}

emili::Solution* emili::MulShake::shake(Solution *s, int n)
{
    emili::Solution *aux = s;
    for(int i = 0; i < shakes.size(); i++){
        s = shakes[i]->perturb(s);
    }
...
emili::Solution* emili::GVNS::search(Solution* initial)
{
        ...
        std::cout << "esto es k max " << k_max << std::endl;
