...
class MaxEvaluations : public emili::Termination
{
protected:
    int max_eva_;
...
public:
    ...
    MaxEvaluations(int max_eva):max_eva_(max_eva), current_eva(0){ }
...
/**
 * Secuencial perturbation
 *
 *
 */

class SecuencialPerturbationSet : public emili::Perturbation
{
protected:
    std::vector<Perturbation*> perturbations;
    //int size;
public:
    SecuencialPerturbationSet(std::vector< Perturbation* > perturbations):perturbations(perturbations) { }
    virtual Solution* perturb(Solution *solution);
    ~SecuencialPerturbationSet() {
        for(std::vector<Perturbation*>::iterator it = perturbations.begin();it!=perturbations.end();++it)
        {
            delete *it;
        }
...
class GeneralTabuSearch: public emili::LocalSearch
{
protected:
    emili::TabuMemory& tabuMemory;
    LocalSearch& ls;
public:
    GeneralTabuSearch(LocalSearch& localsearch,InitialSolution& initialSolutionGenerator ,Termination& terminationcriterion, Neighborhood& neighborh,TabuMemory& tabut):
    emili::LocalSearch(initialSolutionGenerator,terminationcriterion,neighborh),tabuMemory(tabut),ls(localsearch)
    { tabuMemory.setNeighborhood(&neighborh);   }
    virtual emili::Solution* search(emili::Solution* initial);
    virtual emili::Solution* search();
};
...
class MulShake: public Shake
{
protected:
    std::vector< Perturbation* > shakes;
public:
    MulShake(std::vector<Perturbation*> perturbations):shakes(perturbations),Shake(perturbations.size()) {}
    virtual Solution* shake(Solution *s, int n);
};
...
class Removal
{
    ...
    public:
        ...
        virtual void removalArcs(emili::Solution* base) = 0;
        virtual ~Removal() {}
};
...
{
    protected:
        emili::Removal* removalHeuristic;
    public:
        Insertion(emili::Removal* removalHeuristic_){
            removalHeuristic = removalHeuristic_;
        }

        virtual emili::Solution* insertionArcs(emili::Solution* base) = 0;
        virtual ~Insertion() {}
};
