...
#define TERMINATION_EVA "eval"
/*base permutation flowshop solution perturbations */
#define PERTURBATION_RANDOM_FROM_ALL "rndmvf"
...
#define PERTURBATION_SECUENCIAL "secper"
...
prs::Component prs::Builder::buildComponent(int type)
{
    ...
    case COMPONENT_REMOVAL: raw_pointer = buildRemoval();c.setType(COMPONENT_EMPTY);break;
    case COMPONENT_INSERTION: raw_pointer = buildInsertion();c.setType(COMPONENT_EMPTY);break;
...
/*emili::Removal* prs::Builder::buildRemoval()
{
    emili::Removal* nes;
    prs::Component c = retrieveComponent(COMPONENT_REMOVAL);
    while(!c.is(COMPONENT_EMPTY))
    {
        nes.push_back(c.get<emili::Removal>());
        c = retrieveComponent(COMPONENT_REMOVAL,true);
    }
    return nes;
}

emili::Insertion* prs::Builder::buildInsertion()
{
    emili::Insertion* nes;
    prs::Component c = retrieveComponent(COMPONENT_INSERTION);
    while(!c.is(COMPONENT_EMPTY))
    {
        nes.push_back(c.get<emili::Insertion>());
        c = retrieveComponent(COMPONENT_INSERTION,true);
    }
    return nes;
}
*/
...
std::string prs::GeneralParserE::typeName(int type)
{
    ...
    case COMPONENT_REMOVAL: return std::string("Removal");
    case COMPONENT_INSERTION: return std::string("Insertion");
...
emili::LocalSearch* prs::EmBaseBuilder::buildAlgo()
{
    ...
    }else if(tm.checkToken(TABU))
    {
        ...
        if(best >= 0)
        {
            ...
            ls =  new emili::GeneralTabuSearch(*lls,*in,*te,*ne,*tmem);
    }
    else if(tm.checkToken(FIRST))
    {
...
emili::Termination* prs::EmBaseBuilder::buildTermination()
{
    ...
    else if(tm.checkToken(TERMINATION_EVA))
    {
	int evaluations = (int)(250*sqrt(gp.getInstance()->problemSize()));
	std::ostringstream oss;
	oss << "Max evaluations termination. # steps: "<< evaluations;
	printTab(oss.str().c_str());
	term = new emili::MaxEvaluations(evaluations);
    }
...
emili::Perturbation* prs::EmBaseBuilder::buildPerturbation()
{
    ...
    else if(tm.checkToken(PERTURBATION_SECUENCIAL))
    {
       printTab("Secuencial perturbation" );
       std::vector<emili::Perturbation*> ps = this->buildComponentVector<emili::Perturbation>(COMPONENT_PERTURBATION);
       per = new emili::SecuencialPerturbationSet(ps);
    }
...
emili::Shake* prs::EmBaseBuilder::buildShake()
{
    ...
    else if(tm.checkToken(SHAKE_MULTIPLE))
    {
        printTab("Multiple Shake operator ");
        std::vector<emili::Perturbation*> nes = buildComponentVector<emili::Perturbation>(COMPONENT_PERTURBATION);
        sh = new emili::MulShake(nes);
    }
