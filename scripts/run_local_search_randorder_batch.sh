...
# Uso: scripts/run_local_search_randorder_batch.sh <step> <strategy: first|best> <catalog: restricted|unrestricted>
# Corre 15 semillas de búsqueda local pura (sin ILS) con el vecindario nangshift <step> en
# modo randorder, contra el catálogo indicado. Salida en:
#   experiments/local_search/nangshift<step>/<catalog>-shift-rand/data/<strategy>/seedNN/
#
# NO toca experiments/local_search/nangshift<step>/restricted/ ni .../unrestricted/
# (esas carpetas son la baseline histórica de orden fijo, no se sobreescriben).
...
OUTBASE="experiments/local_search/nangshift${STEP}/${CATALOG}-shift-rand/data/${STRATEGY}"
LOG="experiments/local_search/nangshift${STEP}/${CATALOG}-shift-rand/${STRATEGY}_batch.log"
...
for seed in $(seq -w 1 15); do
  ...
  if [ -f "$run_log" ] && grep -q "Found solution" "$run_log" 2>/dev/null; then
    ...
    ./build/emili instances/CERR_Prostate baoimrt 4 csv "$traj" \
      "$STRATEGY" irandomk $CATALOG_TOKEN locmin nangshift "$STEP" randorder \
      rnds "$seed" \
      > "$run_log" 2>&1
